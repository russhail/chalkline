...
async function updateRatings(store, game, homeScore, awayScore) {
  ...
  const { home, away } = await getTeamPair(store, game);
  if (!home || !away) return;
  // What the model believed BEFORE it learns this result. Computed here and
  // only here: a moment from now applyResult will have moved both ratings
  // towards the outcome, and reconstructing the prediction afterwards would be
  // grading the model on knowledge it did not have. It is also why calibration
  // can only run forward — games already rated cannot be recovered.
  const predicted = priceGame(home, away).modelProb;
  ...
  // Written last, and allowed to fail. This is a number for a dashboard, and
  // it sits on the settlement path only because settlement is the one moment
  // the pre-result ratings still exist. Writing it before the rating update
  // put a reporting field ahead of the model: if the column were missing on a
  // cold database the statement would throw, settlement would already have
  // paid out and marked the game settled, and the game could never be retried
  // — so the ratings would freeze at their seeds for the rest of the
  // tournament while the site kept publishing probabilities derived from them,
  // with nothing on /health or the admin queue to show for it. A missing
  // calibration point is a hole in a chart; a frozen model is the whole site.
  await store.query('UPDATE games SET pred_home_prob = $1 WHERE id = $2',
    [predicted, game.id]).catch(() => {});
}
