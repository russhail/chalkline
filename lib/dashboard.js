...
async function teamStats(store, { division = null } = {}) {
  ...
  // How many DIFFERENT pairs of players produce a club's goals. Counting the
  // distinct pairings against the assisted goals they account for says
  // something the top-three share cannot: a side scoring 60 assisted goals
  // through 40 pairings is spreading it around the whole roster, and one
  // scoring 60 through 12 is running the same handful of connections over and
  // over. Star reliance reads the tail of the distribution; this reads the
  // whole graph.
  const conns = await store.query(
    `SELECT team_id, COUNT(*) AS pairs, SUM(n) AS assisted FROM (
       SELECT p.score_team_id AS team_id, p.assist_id, p.scorer_id, COUNT(*) AS n
       FROM points p
       WHERE p.assist_id IS NOT NULL AND p.scorer_id IS NOT NULL ${where}
       GROUP BY p.score_team_id, p.assist_id, p.scorer_id
     ) x GROUP BY team_id`,
    args
  );

  const states = await scoreStates(store, { division });
  ...
  const connByTeam = new Map(conns.map((r) => [Number(r.team_id),
    { pairs: num(r.pairs), assisted: num(r.assisted) }]));
  ...
  const rows = [...byId.values()].map((t) => {
    ...
    const cn = connByTeam.get(t.teamId) || { pairs: 0, assisted: 0 };
    const st = states.get(t.teamId) || blankState();
    return {
      teamId: t.teamId,
      ...
      // Distinct thrower-to-scorer pairings, against the assisted goals they
      // account for. The rate is the share of a club's assisted goals that
      // came from a pairing it had not used before, in effect — high means
      // the connections keep changing, low means the same lines keep firing.
      // Neither is better; a settled first line and a deep rotation are both
      // ways to win a tournament.
      connections: cn.pairs,
      assistedGoals: cn.assisted,
      connectionSpread: pct(cn.pairs, cn.assisted),

      // What the scoreboard said before each point was played. The only
      // dimension in this table that is about the situation rather than the
      // club, and the one that answers whether a side chases or front-runs.
      scoreState: {
        behind: stateSlice(st.behind),
        level: stateSlice(st.level),
        ahead: stateSlice(st.ahead),
      },
      // Points played from 12-12 onward, and how many of them they took. A
      // definition that needs no knowledge of how the game finished.
      tight: { points: st.tight.points, won: st.tight.won,
               pct: pct(st.tight.won, st.tight.points) },
  ...
  // Name breaks the tie. Postgres returns grouped rows in hash order, so two
  // clubs on the same points-played could swap places between two identical
  // requests — the same reshuffling-without-news that topNBy guards against.
...
async function playerTallies(store, { division = null } = {}) {
  ...
  // Added, not assigned. The queries group by (id, name, team), so one player
  // comes back as several rows the moment the feed spells the name differently
  // between games — a dropped accent, an abbreviated surname. Keyed on the id
  // alone, assignment meant the last row silently replaced the earlier ones and
  // that player's earlier goals disappeared from the club's total. The counts
  // are per group; the player is the sum of their groups.
...
// The scoreboard before each point, recovered by walking the chain.
//
// The points table stores the order of scoring but not the running score, and
// it does not need to: replaying a game in sequence gives it back exactly. The
// walk is cheap — the whole tournament is a couple of thousand rows — and it
// is the only way to ask the question this answers, which is whether a club
// behaves differently when it is behind.
//
// Every point is bucketed twice, once for each side, because "ahead" is not a
// property of a point: the same point is the leader's and the chaser's at the
// same time.
function blankState() {
  const side = () => ({ dPoints: 0, breaks: 0, oPoints: 0, holds: 0 });
  return { behind: side(), level: side(), ahead: side(), tight: { points: 0, won: 0 } };
}

function stateSlice(x) {
  return {
    dPoints: x.dPoints, breaks: x.breaks, breakPct: pct(x.breaks, x.dPoints),
    oPoints: x.oPoints, holds: x.holds, holdPct: pct(x.holds, x.oPoints),
  };
}

async function scoreStates(store, { division = null } = {}) {
  const where = division ? 'AND p.division = $1' : '';
  const args = division ? [division] : [];
  // Every point, not just the anchored ones — and that distinction is the
  // whole correctness of this function. A game missing its opening marker has
  // one unanchored point per half sitting in the MIDDLE of the scoreboard, and
  // filtering them out of the query would advance the running score past a
  // goal that was really scored, leaving every later point in that game
  // bucketed against a score short by up to two. score_team_id is populated
  // even when the possession chain is not, so the score can be walked over all
  // of them and only the anchored ones bucketed.
  const rows = await store.query(
    `SELECT p.game_id, p.num, p.o_team_id, p.d_team_id, p.score_team_id, p.anchored
     FROM points p
     WHERE p.score_team_id IS NOT NULL ${where}
     ORDER BY p.game_id, p.num`,
    args
  );

  const out = new Map();
  const get = (id) => {
    if (!out.has(id)) out.set(id, blankState());
    return out.get(id);
  };
  ...
  for (const r of rows) {
    ...
    if (gid !== currentGame) { currentGame = gid; score = new Map(); }
    const o = Number(r.o_team_id), d = Number(r.d_team_id), s = Number(r.score_team_id);

    // Unanchored: the goal counts towards the score, but we do not know which
    // side was on offence, so it cannot be attributed to a bucket.
    if (!r.anchored || !o || !d) { score.set(s, (score.get(s) || 0) + 1); continue; }

    const so = score.get(o) || 0, sd = score.get(d) || 0;

    const oSide = get(o)[bucket(so, sd)];
    oSide.oPoints += 1;
    if (s === o) oSide.holds += 1;

    const dSide = get(d)[bucket(sd, so)];
    dSide.dPoints += 1;
    if (s === d) dSide.breaks += 1;

    // Both sides at twelve or more. In a game to fifteen that is the closing
    // stretch, and unlike "the last three points" it can be decided from the
    // scoreboard as it stood, without knowing how the game turned out.
    if (so >= 12 && sd >= 12) {
      for (const id of [o, d]) {
        const t = get(id).tight;
        t.points += 1;
        if (s === id) t.won += 1;
      }
    }
...
// Is the model's 70% actually 70%?
//
// Folded onto the favourite, so every game contributes to one bucket and the
// buckets run 50-100 rather than 0-100 — a 30% prediction and a 70% one are
// the same claim seen from opposite ends, and splitting them would halve the
// sample for no gain.
//
// Only games carrying a prediction recorded at settlement can appear here.
// Recomputing an old game's probability from today's ratings would be scoring
// the model on the answers, so the record starts when this shipped and grows.
async function calibration(store, { division = null } = {}) {
  const args = division ? [division] : [];
  const rows = await store.query(
    `SELECT pred_home_prob AS p, home_score AS hs, away_score AS a_s, division
     FROM games
     WHERE pred_home_prob IS NOT NULL AND settled = TRUE AND voided = FALSE
  ...
  const buckets = edges.slice(0, -1).map((lo, i) => ({
    lo, hi: edges[i + 1] > 1 ? 1 : edges[i + 1],
    label: `${Math.round(lo * 100)}\u2013${Math.round(Math.min(edges[i + 1], 1) * 100)}%`,
    games: 0, predictedSum: 0, correct: 0,
  }));

  let n = 0, brier = 0, correct = 0;
  for (const r of rows) {
    const p = Number(r.p);
    if (!Number.isFinite(p)) continue;
    const homeWon = Number(r.hs) > Number(r.a_s);
    // The favourite's probability, and whether the favourite won.
    const fav = Math.max(p, 1 - p);
    const favWon = (p >= 0.5) === homeWon;
    const b = buckets.find((x) => fav >= x.lo && fav < x.hi) || buckets[buckets.length - 1];
    b.games += 1; b.predictedSum += fav; if (favWon) b.correct += 1;
    // Brier score over the raw home probability: mean squared error, where 0
    // is perfect and 0.25 is what you get by always saying "coin flip".
    brier += (p - (homeWon ? 1 : 0)) ** 2;
    if (favWon) correct += 1;
    n += 1;
  }
  ...
  return {
    games: n,
    // Null rather than 0 on an empty record, so a page cannot render a
    // flawless-looking score for a model that has predicted nothing.
    brier: n ? Math.round((brier / n) * 1e4) / 1e4 : null,
    favouriteAccuracy: pct(correct, n),
    buckets: buckets.map((b) => ({
      label: b.label,
      games: b.games,
      predicted: b.games ? Math.round((b.predictedSum / b.games) * 1000) / 10 : null,
      actual: pct(b.correct, b.games),
    })),
  };
}
...
const PROFILE_STATS = [
  ...
  // Ungated, for the same reason the page's timeout view is: every club's
  // timeout sample is small, so a threshold would split small from slightly
  // smaller and present it as a real distinction. The count travels in the
  // sub-line instead, which says the same thing without hiding the row.
  { key: 'timeoutConversion', label: 'After a timeout', unit: '%', sample: 'timeouts', min: 0 },
  { key: 'timeoutBreakRate', label: 'Timeout breaks', unit: '%', sample: 'timeouts', min: 0 },
  ...
  // Neutral: a settled first line and a deep rotation are both ways to win.
  { key: 'connectionSpread', label: 'Connection depth', unit: '%',
    sample: 'assistedGoals', min: 10, neutral: true },
  // Break rate split by what the scoreboard said. Gated on the points played
  // in that particular state, which is the only sample that bears on it — a
  // club that has led all tournament has no meaningful "behind" number.
  { key: 'scoreState.behind.breakPct', label: 'Breaks when behind', unit: '%',
    sample: 'scoreState.behind.dPoints', min: 8 },
  { key: 'scoreState.ahead.breakPct', label: 'Breaks when ahead', unit: '%',
    sample: 'scoreState.ahead.dPoints', min: 8 },
  { key: 'tight.pct', label: 'Points from 12-12', unit: '%',
    sample: 'tight.points', min: 5 },
];
...
//
// Seeds are set before a disc is thrown; the rating has watched every result
// since. So the column worth looking at is the gap between the two — that is
// where the tournament has disagreed with whoever did the seeding.
//
// rd is Glicko's uncertainty and travels with every row, because a club the
// model has barely seen is not really ranked yet, and a position without that
// caveat invites an argument the number cannot support.
//
// Name breaks a rating tie, so two clubs on the same number do not swap places
// between requests. A ranking that reshuffles while nothing has happened reads
// as movement, which is the one thing a ranking must never fake.
async function rankings(store, { division = null } = {}) {
  const args = division ? [division] : [];
  const rows = await store.query(
    `SELECT id, name, abbreviation, division, country, seed, rating, rd, played
     FROM teams ${division ? 'WHERE division = $1' : ''}
  ...
  for (const list of byDivision.values()) {
    list.forEach((t, i) => {
      ...
      out.push({
        ...
        rd: Math.round(Number(t.rd)),
        played: num(t.played),
        position,
        // Positive means the model has them higher than the seeding did.
        seedDelta: t.seed ? Number(t.seed) - position : null,
        // Under two games the rating is still mostly the seed it started from,
        // so the position is not yet the model's opinion — it is the seeder's.
        provisional: num(t.played) < 2,
      });
    });
  }
  return out;
