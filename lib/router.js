...
import { priceGame, spreadLadder } from './model.js';
...
// YYYY-MM-DD, the only shape any day parameter may take.
...
async function handle(req, opts = {}) {
  ...
  try {
    ...
    if (method === 'GET' && path === '/games') {
      ...
      if (day && !DAY_RE.test(day)) return fail(400, 'Bad day.');
    ...
    if (method === 'GET' && path === '/live') {
      ...
      const games = rows.map((g) => {
        ...
        return {
          ...
          decided,
          suspendedFor,
          // The in-play probability, recomputed from the score and how much of
          // the game is left. Served as a probability and nothing else: this
          // used to carry a decimal price alongside, which is meaningless on a
          // site nobody can bet on.
          home: { id: home.id, name: home.name, country: home.country, seed: home.seed,
    ...
    if (method === 'GET' && path === '/results') {
      const division = url.searchParams.get('division');
      const day = url.searchParams.get('day');
      // The day is interpolated into timestamp bounds, and Postgres rejects a
      // malformed one outright ("invalid input syntax for type timestamp with
      // time zone") where SQLite's string comparison just matches nothing. So
      // the shape is checked here rather than discovered in production.
      if (day && !DAY_RE.test(day)) return fail(400, 'Bad day.');
