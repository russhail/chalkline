...
import { sync, syncLive, syncGameDetail, clearDerivedPoints, openGames, liveGames, gameDays, poolsNeedingScores, getMeta, setMeta } from './sync.js';
...
import { teamStats, playerLeaders, coverage } from './dashboard.js';
...
async function priceBoard(store, rows) {
  ...
  return rows
    .map((g) => {
      ...
      const p = priceGame(home, away, g.stake_home, g.stake_away);
      return {
        id: g.id,
        ...
        home: { id: home.id, name: home.name, country: home.country, seed: home.seed,
                odds: p.home.decimal, prob: p.home.prob, staked: g.stake_home },
        away: { id: away.id, name: away.name, country: away.country, seed: away.seed,
                odds: p.away.decimal, prob: p.away.prob, staked: g.stake_away },
      };
    })
    .filter(Boolean);
}
...
async function handle(req, opts = {}) {
  ...
  try {
    ...
    if (method === 'POST' && path === '/signup') {
      const gate = await limit.hit(store, `signup:${who}`, limit.WINDOW.SIGNUP, now);
      if (!gate.allowed) {
        return fail(429, `Too many accounts created from here. Try again in ${Math.ceil(gate.retryAfter / 60)} minutes.`);
      }
      const res = await auth.createUser(store, { ...body, now });
      if (!res.ok) return fail(400, res.errors.join(' '));
      const session = await auth.login(store, {
        displayName: res.displayName, recoveryCode: res.recoveryCode, now,
      });
      // The recovery code is returned here and nowhere else, ever.
      return json(200, {
        ok: true,
        user: session.user,
    ...
    if (method === 'GET' && path === '/me') {
      ...
      await betting.topUpStuck(store, { clock: () => now, userId: me.id }).catch(() => {});
    ...
    if (method === 'POST' && path === '/bet') {
      const user = requireUser();
      const gate = await limit.hit(store, `bet:${user.id}`, limit.WINDOW.BET, now);
      if (!gate.allowed) return fail(429, 'Slow down a moment.');
      const res = await betting.placeBet(store, {
        userId: user.id, gameId: Number(body.gameId), side: body.side,
        stake: Number(body.stake),
        market: body.market === 'spread' ? 'spread' : 'moneyline',
        line: body.line, clock: () => now,
      });
      return ok({ ok: true, bet: res });
    }

    if (method === 'GET' && path === '/mybets') {
      const user = requireUser();
      const bets = await store.query(
        `SELECT b.*, g.starts_at, g.status AS game_status, g.home_score, g.away_score,
                g.live_home_score, g.live_away_score,
                h.name AS home_name, a.name AS away_name
         FROM bets b JOIN games g ON g.id = b.game_id
         LEFT JOIN teams h ON h.id = g.home_team_id
         LEFT JOIN teams a ON a.id = g.away_team_id
         WHERE b.user_id = $1 ORDER BY b.placed_at DESC LIMIT 200`,
        [user.id]
      );
      return ok({
        bets: bets.map((b) => ({
          ...b,
          starts_at: toIso(b.starts_at),
          placed_at: toIso(b.placed_at),
          settled_at: toIso(b.settled_at),
        })),
        bankroll: Math.round(user.bankroll * 100) / 100,
      });
    }

    if (method === 'GET' && path === '/leaderboard') {
      return json(200, await betting.leaderboards(store, { day: url.searchParams.get('day') }),
        { 'Cache-Control': SHARED_CACHE });
    }
    ...
    if (path === '/sync' && (method === 'POST' || method === 'GET')) {
      ...
      await betting.topUpStuck(store, { clock: () => now });
    ...
    if (method === 'GET' && path === '/tick') {
      ...
      await betting.topUpStuck(store, { clock: () => now }).catch(() => {});
      return ok({ ...result, detail });
    }
    ...
    if (method === 'GET' && path === '/stats') {
      ...
      const [teams, players, cov] = await Promise.all([
        teamStats(store, { division }),
        playerLeaders(store, { division }),
        coverage(store),
      ]);
      return json(200, { teams, players, coverage: cov },
