...
async function syncGames(store, games, now = Date.now()) {
  ...
  for (const g of games) {
    ...
    if (!existing) {
      ...
      // And by the same argument it can be already FINISHED the first time we
      // see it: a cold database replaying a tournament, or a game that both
      // started and ended between two syncs. This check used to live only on
      // the update path, below, so such a game inserted unsettled and unrated
      // — its bets never graded, the model never learned from the result, and
      // it never reached the results page. The condition is deliberately the
      // same one used there; a score alone is not enough, because the feed
      // publishes running scores too.
...
// Games with a result. The mirror of openGames — that one answers "what can I
// still watch", this one answers "what did I miss". Ordered newest first,
// because on a results page the thing you want is the game that just ended.
//
// Gated on status, not on "has a score". The standings file publishes the
// RUNNING score of a game in progress (see wfdf.js) and syncGames writes it
// straight into home_score/away_score, so a score-only predicate served a
// game still being played as though it had finished — 6-3 rendered as a final
// result for a game still on the pitch.
//
// Status rather than `settled`, which was the first attempt: settlement is
// deliberately withheld from forfeits and from anything held for review, and
// those are real results a reader wants to see. Status is the feed's own
// statement that the game is over, and it is already what syncGames trusts
// before it will settle anything.
//
// `OR voided` because an abandoned game never reaches 'final' — it stops
// mid-play and an admin rules on it — and a game called off in a storm is
// still something that happened, with a score it stood at when it stopped.
// Between them the two clauses mean "somebody has decided this is over",
// which is the actual question, where the score column only ever answered
// "has anybody scored".
//
// starts_at can be null when the feed omits time_utc; those are excluded
// rather than rendered under an "Invalid Date" heading.
async function finishedGames(store, { limit = 500, day = null } = {}) {
  const args = [];
  let dayClause = '';
  if (day) {
    args.push(`${day}T00:00:00.000Z`, `${day}T23:59:59.999Z`);
    dayClause = 'AND g.starts_at >= $1 AND g.starts_at <= $2';
  }
  args.push(limit);
  const rows = await store.query(
    `SELECT g.id, g.division, g.pool_name, g.starts_at, g.home_score, g.away_score,
            g.voided, g.void_reason, g.forfeit, g.home_team_id, g.away_team_id,
            h.name AS home_name, h.country AS home_country, h.seed AS home_seed,
            a.name AS away_name, a.country AS away_country, a.seed AS away_seed
     FROM games g
     LEFT JOIN teams h ON h.id = g.home_team_id
     LEFT JOIN teams a ON a.id = g.away_team_id
     WHERE (g.status = 'final' OR g.voided = TRUE) AND g.starts_at IS NOT NULL
       AND g.home_score IS NOT NULL AND g.away_score IS NOT NULL
       AND g.home_team_id > 0 AND g.away_team_id > 0 ${dayClause}
  ...
  return rows.map((g) => ({
    ...
    voided: Boolean(g.voided),
    voidReason: g.void_reason || null,
    forfeit: Boolean(g.forfeit),
    score: { home: Number(g.home_score), away: Number(g.away_score) },
    home: { id: Number(g.home_team_id), name: g.home_name,
            country: g.home_country || null, seed: g.home_seed ?? null },
    away: { id: Number(g.away_team_id), name: g.away_name,
            country: g.away_country || null, seed: g.away_seed ?? null },
  }));
}

// Days that have results, newest first — the opposite order to gameDays,
// which counts forward to the next fixture.
async function resultDays(store) {
  ...
  const rows = await store.query(
    `SELECT ${dayExpr} AS day, COUNT(*) AS games
     FROM games
     WHERE (status = 'final' OR voided = TRUE) AND starts_at IS NOT NULL
...
export {
  finishedGames,
  resultDays, sync, syncLive, syncTeams, syncGames, settleFinished, openGames, liveGames, gameDays,
...
async function syncGameDetail(store, { fetcher = fetchGameDetail, limit = DETAIL_BATCH } = {}) {
  // Games that are OVER, not games that have a score.
  //
  // This used to select on `home_score IS NOT NULL`, and that was quietly
  // ruinous. The standings file publishes the RUNNING score of a game in
  // progress and syncGames writes it into home_score, so a game that had
  // reached 3-1 looked finished. The ingest then fetched its per-game file,
  // found the four goals played so far, wrote them, and set detail_synced —
  // and because the ingest skips anything already marked done, the twenty
  // goals scored after that moment were never fetched. Every game caught
  // mid-play was permanently truncated, and every statistic on the site was
  // computed over the fragment.
  //
  // Gating on `settled` would be wrong in the other direction: settlement is
  // withheld from forfeits and from anything held for review. Status is the
  // feed's own statement that the game is finished, which is the actual
  // question, and it is what syncGames already trusts before settling.
  const pending = await store.query(
    `SELECT id, division, home_score, away_score, forfeit FROM games
  ...
  for (const g of pending) {
    try {
      ...
      for (const t of timeouts) {
        await store.query(
          `INSERT INTO timeouts (game_id, time_s, team_id, converted, was_break)
           VALUES ($1,$2,$3,$4,$5) ON CONFLICT (game_id, time_s, team_id) DO NOTHING`,
          [g.id, t.timeS, t.teamId, t.converted, t.wasBreak]
        );
      }

      // Belt and braces on top of the status check: the goals we derived must
      // account for the final score. If the per-game file is still catching up
      // with the standings — which is exactly how the truncation happened —
      // the counts disagree, and marking the game done would freeze the gap in
      // permanently. Leaving detail_synced false costs one re-fetch next tick
      // and is self-healing; the points already written are deduplicated by
      // the ON CONFLICT above.
      //
      // A forfeit is exempt: it is awarded, not played, so its file carries no
      // goals and never will.
      const expected = Number(g.home_score) + Number(g.away_score);
      if (!g.forfeit && points.length !== expected) {
        deferred += 1;
        continue;
      }
  ...
  // `deferred` is games whose file did not yet account for the final score.
  // Worth reporting rather than hiding: a number that stays high means the
  // feed and the standings are persistently out of step, which is a different
  // problem from a game failing to fetch.
  return { pending: pending.length, ingested, pointsWritten, unanchored, deferred, failures };
}
