...
function buildSchema(T) {
  return [
    ...
    // was_break splits a converted timeout into the break it won and the hold
    // it merely kept. Null means either "not converted" or "converted on a
    // point we could not anchor" — see timeoutOutcomes in gamestats.js.
    ...
    `CREATE TABLE IF NOT EXISTS games (
      ...
      -- What the model believed before it saw this result. Written once, at
      -- settlement, and never recomputed: a prediction graded with knowledge
      -- of the outcome is not a prediction.
      pred_home_prob ${T.NUM}
    )`,
...
function buildMigrations(T) {
  return [
    ...
    // Splits a converted timeout into a break and a hold. Existing rows stay
    // null until the points are re-derived, which is the honest state: the
    // flag was never computed for them, and null already means "cannot say".
    `ALTER TABLE timeouts ADD COLUMN was_break ${T.BOOL}`,
    // Forward-only by necessity. Games already rated have moved the ratings
    // that produced them, so there is no honest way to backfill this.
    `ALTER TABLE games ADD COLUMN pred_home_prob ${T.NUM}`,
  ];
}
