...
//
// Winning that point is two different achievements wearing one word. A side
// that calls timeout while on defence and then scores has taken a break; one
// that calls on offence and scores has held the disc it already had. Both are
// "converted", and the first is much harder — so the break flag travels with
// the outcome rather than being averaged into it.
//
// wasBreak is null in two distinct cases, and they are not the same: the
// timeout was not converted at all, or it was converted on a point whose
// possession chain was never anchored, so we cannot say which line was out.
// Callers must not read null as false.
function timeoutOutcomes(points, timeouts) {
  return timeouts.map((t) => {
    const next = points.find((p) => p.timeS >= t.timeS);
    const converted = next ? next.scoreTeam === t.teamId : null;
    return {
      ...t,
      converted,
      // The scorer of an anchored break is by definition the side that was on
      // defence, so the point's own flag answers it without a second lookup.
      wasBreak: converted === true ? next.isBreak : null,
    };
  });
}
